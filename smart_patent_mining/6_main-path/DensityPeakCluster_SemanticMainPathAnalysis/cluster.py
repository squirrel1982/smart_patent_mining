...
def load_mainPathLen(mainPathLen_f):
    ...
    with open(mainPathLen_f, 'r') as f:
        node_num_of_path_list = pickle.load(f)
    return node_num_of_path_list
...
def local_hybrid_density(max_id, distances, pathLen_list,dc, local_den_weight, path_len_weight,\
                         guass=True, cutoff=False):
    '''
    Compute all points' local density
...
class DensityPeakCluster(object):
    ...
    def local_hybrid_density(self, data_load_func, distance_f,normalized_path_len, \
        ...
        if dc is None:
            dc = select_dc(max_id, max_dis, min_dis,
                           distances, auto=auto_select_dc)
        rho = local_hybrid_density(max_id, distances,pathLen_list,dc,local_den_weight = local_den_weight, path_len_weight = path_len_weight)
        return distances, max_dis, min_dis, max_id, rho
